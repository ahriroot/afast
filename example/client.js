...
//  * @param {{id:number,name:string,age:number,hobbies:Array<{id:number,name:string}>,tags:Array<string>,gender:boolean|null,sex:{_type:0}|{_type:1,id:number}|{_type:2,name:string}}} request
//  * @returns {{sex:{_type:0}|{_type:1,id:number}|{_type:2,name:string},id:number,name:string,age:number,hobbies:Array<{id:number,name:string}>,tags:Array<string>,gender:boolean|null}}
//  */
// get_user: async (request) => {if(request.age===undefined||request.age===null) throw new AFastValidateError("name is required");if(request.age<1)throw new AFastValidateError("name must be at least 1 character long");if(request.age>100)throw new AFastValidateError("name must be at most 10 characters long");if(request.sex._type===undefined)throw new AFastValidateError('Missing _type field in request.sex');if(![0,1,2].includes(request.sex._type))throw new AFastValidateError(`Invalid _type field in request.sex: ${request.sex._type} not in [0,1,2]`);const _b1 = new AFastByteBuffer();const _header = await this._header();_b1.pU32(_header.id);_b1.pU32(0+this.offset);_b1.pI64(request.id);_b1.pS(request.name);_b1.pU32(request.age);_b1.pU32(request.hobbies.length);for(let _i1 of request.hobbies){_b1.pI64(_i1.id);_b1.pS(_i1.name);}_b1.pU32(request.tags.length);for(let _i1 of request.tags){_b1.pS(_i1);}if(request.gender===null){_b1.pU8(0);}else{_b1.pU8(1);_b1.pB(request.gender);}_b1.pU32(request.sex._type);switch(request.sex._type){case 0:break;case 1:_b1.pI64(request.sex.id);break;case 2:_b1.pS(request.sex.name);break;}const _b2 = new AFastByteReader(await this._call(_b1.tU8A()));_b2.rI32();const response={sex:(() => {let _type = _b2.rU32();switch(_type) {case 0: return ({ _type: 0, value: {} });case 1: return ({ _type: 1, id: _b2.rI64() });case 2: return ({ _type: 2, name: _b2.rS() });}})(),id:_b2.rI64(),name:_b2.rS(),age:_b2.rU32(),hobbies:Array.from({length:_b2.rU32()},()=>({id:_b2.rI64(),name:_b2.rS()})),tags:Array.from({length:_b2.rU32()},()=>(_b2.rS())),gender:_b2.rU8()===1?_b2.rB():null};return response;}
// }
// }
// }

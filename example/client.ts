...
export type ClientHeader=()=>Promise<{token:string}>
export type ClientHook=(data:{token:string})=>Promise<any>
export type Options={header:ClientHeader,hook?:ClientHook,call:ClientCall,[key:string|number|symbol]:any}
...
/**
 * Create client
 * @param {{header:()=>Promise<{token:string}>,call:(buf:Uint8Array)=>Promise<Uint8Array>,}} options
 */
constructor(options:Options){this._options=options;if(!options.header){throw new Error('header is required');};this._header=options.header;if(!options.call){throw new Error('call is required');};this._hook=options.hook;this._call=options.call}
api = {
/**
 * Get user by id
 ...
 */
get_id:async(request:{id:number}):Promise<{id:number,name:string}>=>{const _b1=new AFastByteBuffer();const _header=await this._header();_b1.pS(_header.token);_b1.pU32(1+this.offset);_b1.pI64(request.id);const _b2=new AFastByteReader(await this._call(_b1.tU8A()));_b2.rI32();const _header2={token:_b2.rS()};this._hook?.(_header2);const response={id:_b2.rI64(),name:_b2.rS()};return response as any;},
user: {
/**
 ...
 */
get_user:async(request:{id:number,name:string,age:number,hobbies:{id:number,name:string}[],tags:string[],gender:boolean|null,sex:{_type:0}|{_type: 1,_0:number,_1:string}|{_type: 2,id:number}|{_type: 3,name:string}}):Promise<{sex:{_type:0}|{_type: 1,_0:number,_1:string}|{_type: 2,id:number}|{_type: 3,name:string},id:number,name:string,age:number,hobbies:{id:number,name:string}[],tags:string[],gender:boolean|null}>=>{if(request.age===undefined||request.age===null) throw new AFastValidateError("age is required");if(request.age<1)throw new AFastValidateError("age must be at least 1");if(request.age>256)throw new AFastValidateError("age must be at most 256");if(request.sex._type===undefined)throw new AFastValidateError('Missing _type field in request.sex');if(![0,1,2,3].includes(request.sex._type))throw new AFastValidateError(`Invalid _type field in request.sex: ${request.sex._type} not in [0,1,2,3]`);const _b1=new AFastByteBuffer();const _header=await this._header();_b1.pS(_header.token);_b1.pU32(0+this.offset);_b1.pI64(request.id);_b1.pS(request.name);_b1.pU32(request.age);_b1.pU32(request.hobbies.length);for(let _i1 of request.hobbies){_b1.pI64(_i1.id);_b1.pS(_i1.name);}_b1.pU32(request.tags.length);for(let _i1 of request.tags){_b1.pS(_i1);}if(request.gender===null){_b1.pU8(0);}else{_b1.pU8(1);_b1.pB(request.gender);}_b1.pU32(request.sex._type);switch(request.sex._type){case 0:break;case 1:_b1.pI32(request.sex._0);_b1.pS(request.sex._1);break;case 2:_b1.pI64(request.sex.id);break;case 3:_b1.pS(request.sex.name);break;}const _b2=new AFastByteReader(await this._call(_b1.tU8A()));_b2.rI32();const _header2={token:_b2.rS()};this._hook?.(_header2);const response={sex:(() => {let _type = _b2.rU32();switch(_type) {case 0: return ({ _type: 0 });case 1: return ({ _type: 1, _0: _b2.rI32(), _1: _b2.rS() });case 2: return ({ _type: 2, id: _b2.rI64() });case 3: return ({ _type: 3, name: _b2.rS() });}})(),id:_b2.rI64(),name:_b2.rS(),age:_b2.rU32(),hobbies:Array.from({length:_b2.rU32()},()=>({id:_b2.rI64(),name:_b2.rS()})),tags:Array.from({length:_b2.rU32()},()=>(_b2.rS())),gender:_b2.rU8()===1?_b2.rB():null};return response as any;}
}
}
}
